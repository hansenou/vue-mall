          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                ...
                :key="SaleAttrList.id"
              >
                <dt class="title">{{ SaleAttrList.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: SaleAttrValueList.isChecked == 1 }"
                  v-for="SaleAttrValueList in SaleAttrList.spuSaleAttrValueList"
                  :key="SaleAttrValueList.id"
                  @click="
                    changeActive(
                      SaleAttrValueList,
                      SaleAttrList.spuSaleAttrValueList
                    )
                  "
                >
                  {{ SaleAttrValueList.saleAttrValueName }}
                </dd>
              </dl>
...
export default {
  ...
  mounted() {
    //派发action获取产品详情的信息
    this.$store.dispatch("getDeatilInfo", this.$route.params.skuId);
  },
  computed: {
  ...
  methods: {
    ...
    async addShopcar() {
      //1、发请求--将产品加入到数据库
      //2、需要知道请求成功还是失败，成功进行路由跳转，失败就给用户提示
      try {
        //成功
